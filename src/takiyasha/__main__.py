...
if __name__ == '__main__':
    ...
    openfile_kwargs = vars(cli.ap.parse_args())

    srcpaths: list[cli.Path] = openfile_kwargs.pop('srcpaths')
    destdir: cli.Path = openfile_kwargs.pop('destdir')
    ...
    if len(tasked_paths) == 0:
        cli.print_stdout('错误：输入路径中没有任何受支持的文件')
        cli.print_stdout("提示：如果输入路径中存在目录，您应当加上 '-r' 或 '--recursive' 选项")
        sys.exit(1)

    if not openfile_kwargs['probe_content']:
        cli.print_stdout("警告：您添加了 '--faster' 选项")
        cli.print_stdout('将会跳过文件内容，仅根据文件名判断加密类型')
    ...
    if try_fallback:
        ...
        cli.print_stdout('将在单个文件首次解密失败时，使用旧方案再次尝试')
    ...
    if enable_multiprocessing:
        ...
        for cursrcfile, curdstdir in tasked_paths:
            ...
            p = mp.Process(target=cli.task, kwargs=params, name=str(cursrcfile))
            p.start()
            task_procs.append(p)
